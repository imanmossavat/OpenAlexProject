...
export default function useExperimentRootPreference() {
  ...
  const fetchRoot = useCallback(async () => {
    setRootLoading(true)
    setRootError(null)
    const res = await apiClient('GET', `${endpoints.settings}/experiment-root`)
    if (res.error) {
  ...
  const saveRoot = useCallback(async () => {
    ...
    if (!isAbsolutePath(rootInput)) {
      setRootError('Default experiment path must be absolute.')
      return false
    }
    setRootSaving(true)
    setRootError(null)
    const payload = { path: rootInput.trim() }
    const res = await apiClient('PUT', `${endpoints.settings}/experiment-root`, payload)
    if (res.error) {
      setRootError(res.error)
      setRootSaving(false)
      return false
  ...
  const resetRoot = useCallback(async () => {
    setRootSaving(true)
    setRootError(null)
    const res = await apiClient('PUT', `${endpoints.settings}/experiment-root`, { path: null })
    if (res.error) {
      setRootError(res.error)
    } else {
      setDefaultRoot(null)
